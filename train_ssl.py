...
def get_args_parser():
    ...
    parser.add_argument('--head', default='DINOHead', type=str, help="""Name of head to train with.""")
    ...
    parser.add_argument('--continuous_repeat', type=utils.bool_flag, default=False, help="""""")
    parser.add_argument('--use_ln_in_head', type=utils.bool_flag, default=False, help="""""")
...
def train_svt(args):
    ...
    elif args.continuous_repeat:
        sampler = datasets.ContinuousRepeatSampler(dataset, batch_size=args.batch_size, world_size=args.world_size)
    else:
    ...
    Memory = getattr(utils, args.memory) if args.memory else None
    print('Memory', Memory)
    ...
    student = Wrapper(student,
         ...
         memory=Memory(args.batch_size_per_gpu, args.maxlen) if args.memory else None,
    ...
    for it, (images, indices, clip_indices, video_indices, *_) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        if fp16_scaler is None:
            ...
            utils.cancel_gradients_last_layer(epoch, student,
                                              args.freeze_last_layer)
            try:
                head_gradnorm = {f'head_lin{0}_gradnorm': torch.norm(student.module.head.mlp[0].weight.grad)}
            except:
                head_gradnorm = {}
