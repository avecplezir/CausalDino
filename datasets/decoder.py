...
def decode_events(
    ...
    if temporal_aug:
        ...
        num_global_frames = num_frames
        num_local_frames = num_frames
        if num_clips_global == 2:
            global_1 = temporal_sampling(frames, 0, max_len - 5, num_global_frames)
            global_2 = temporal_sampling(frames, 5, max_len, num_global_frames)
            global_samples = [global_1, global_2]
        else:
            global_samples = []
            for _ in range(num_clips_global):
                idx = np.random.randint(0, 5)
                global_samples.append(temporal_sampling(frames, idx, max_len - idx, num_global_frames))
        ...
        if random_sampling:
            ...
            indices = np.random.choice(np.arange(0, n_parts - 1), replace=False,
                                       size=num_clips_global + local_crops_number)
