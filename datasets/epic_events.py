...
class ContinuousRepeatSampler(ContinuousBeg2EndSampler):
    def __iter__(self):
        dataset_length = len(self.data_source)
        sampler_idx = 0
        current_choices = np.random.choice(np.arange(dataset_length), size=self.batch_size, replace=False)
        current_choices_np = np.array(list(current_choices))
        repeat = 8
        while True:
            for i in range(repeat):
                for j, data_idx in enumerate(current_choices_np):
                    if j % repeat == 0:
                        new_data_idx = np.random.choice(np.arange(dataset_length))
                        idx = i + j
                        if idx < self.batch_size:
                            current_choices_np[idx] = new_data_idx
                    yield data_idx
                    sampler_idx += 1
