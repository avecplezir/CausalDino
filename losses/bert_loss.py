...
class BertLoss(FeatureLoss):
    def forward(self, student_output, teacher_output, epoch, **kwargs):
        ...
        """
        s_pred_logits_list, inverse_masks = student_output
        t_enc_logits, *_ = teacher_output

        temp = self.teacher_temp_schedule[epoch]
        t_enc_proba = F.softmax((t_enc_logits - self.center) / temp, dim=-1)

        CE_fe = self.compute_loss_fe(s_pred_logits_list, t_enc_proba, inverse_masks)
...
class GPTLoss(BertLoss):
    def compute_loss_fe(self, s_pred_logits, t_enc_proba, inverse_masks=None):
        s_pred_log = F.log_softmax(s_pred_logits / self.student_temp, dim=-1)
        loss = -torch.sum(t_enc_proba[:, 1:] * s_pred_log[:, :-1], dim=-1)
        return loss.mean()


class TELoss(BertLoss):
    def compute_loss_fe(self, s_pred_future_logits_list, t_enc_proba, inverse_masks=None):
        total_loss = 0
        n_loss_terms = 0
