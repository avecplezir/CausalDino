...
class GPTSim2Loss(nn.Module):
    ...
    def forward(self, student_output, teacher_output, epoch, student=None, teacher=None):
        ...
        teacher_logits_predictions2 = (teacher_logits_predictions - self.prediction_center) / temp
        labels_predictions = F.softmax(student_logits, dim=-1)
        CE_pf = torch.sum(-labels_predictions[:, 1:] * F.log_softmax(teacher_logits_predictions2[:, :-1], dim=-1), dim=-1)
