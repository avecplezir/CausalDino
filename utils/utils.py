...
class NeuralPatchMemory:
    def __init__(self, batch_size, maxlen, args=None, memory_dim=256):
        self.batch_size = batch_size
        self.maxlen = maxlen
        self.memory = deque(maxlen=self.maxlen)
        self.memory_mask = deque(maxlen=self.maxlen)
        self.current_video_indices = -torch.ones(batch_size)
        self.memory_gpt = GPT(in_dim=memory_dim, out_dim=memory_dim,
                              block_size=self.maxlen*self.args.n_global_views,
                              model_type='gpt-micro-256-one', maskemb=True)
        self.new_video_token = None
